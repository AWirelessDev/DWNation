...
const DropDown = ({
  ...
  const customStyles = {
    ...
    control: (styles, { isDisabled }) => {
      return {
        ...styles,
        backgroundColor: pendingChanges?.includes(id)
          ? "#FDF4D1"
          : isDisabled
          ? "#e9ecef"
          : "#FFF",
  ...
  if(isMulti) {
    selectedValue = options?.filter((item) => value?.includes(item[optionValue]));
  } else {
    selectedValue = value ? options?.find((item) => item[optionValue]?.toString() === value?.toString()) : null;
  }

  const hasError = !(isDisabled || editable) && errors?.filter(error => error.key === name );
  return (
    <>
      ...
      <Select
        className={`mt-2 ${className} ${hasError?.length ? 'error-message': ''}`}
        components={
          editable
            ? {
